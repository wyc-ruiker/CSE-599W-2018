...
class AddByConstOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        """TODO: Your code here"""
        const_shape = (1,)
        return broadcast_rule(input_shapes[0], const_shape)
...
class MatMulOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        ...
        if node.matmul_attr_trans_A:
            l = (input_shapes[0][1], input_shapes[0][0])
        if node.matmul_attr_trans_B:
            r = (input_shapes[1][1], input_shapes[1][0])
        assert(l[1] == r[0])
        return (l[0], r[1])
...
class Executor(object):
    ...
    def memory_plan(self, feed_shapes):
        ...
        for node in self.topo_order:
            if node in feed_shapes:
                continue
            self.node_to_arr_map[node] = tvm.ndarray.empty(self.node_to_shape_map[node], dtype='float32', ctx=self.ctx)
